        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-6 py-4 pb-24">
            ...
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="font-arabic font-bold text-base text-[var(--text)]">المزادات النشطة <span className="text-[var(--text-muted)] text-xs">({items.length})</span></h2>
                </div>
            ...
            {loading ? (
                ...
                <div className="bg-[var(--surface)] rounded-2xl p-10 text-center border border-[var(--border)]">
                    ...
                    <p className="text-[var(--text-muted)] font-arabic-body mb-3">لا توجد مزادات نشطة الآن</p>
                    <Link to="/post" className="inline-block bg-[var(--primary)] text-[var(--primary-fg)] px-5 py-2 rounded-full font-arabic font-bold text-sm">أنشئ أول مزاد</Link>
                </div>
            ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        <div data-testid={`auction-card-${listing.id}`} className="bg-[var(--surface)] rounded-2xl overflow-hidden border border-[var(--border)] hover:border-[var(--accent)] hover:shadow-lg transition-all flex flex-col">
            <Link to={`/listing/${listing.id}`} className="block aspect-[5/3] bg-[var(--surface-elevated)] overflow-hidden relative">
                {listing.images?.[0] ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-[var(--text-muted)] font-arabic">لا توجد صورة</div>
                )}
                ...
                <div className="p-4 space-y-4">
                    <div className="bg-[var(--surface-elevated)] rounded-xl p-3 flex items-center justify-between">
                        ...
                        <div className="text-right">
                            <div className="text-[10px] text-[var(--text-muted)] font-arabic-body">عدد المزايدات</div>
                            <div className="font-arabic font-black text-xl text-[var(--text)]">{bids.length}</div>
                        </div>
                    </div>

                    <form onSubmit={submit} className="space-y-2">
                        <label className="block text-xs font-arabic font-bold text-[var(--text)]">مبلغ المزايدة <span className="text-[var(--text-muted)] font-arabic-body">(الحد الأدنى: {minRequired.toLocaleString()})</span></label>
                    ...
                    {bids.length > 0 && (
                        <div>
                            <div className="text-xs font-arabic font-bold text-[var(--text)] mb-2">تاريخ المزايدات</div>
                                ...
                                {bids.map((b, i) => (
                                    <div key={b.id} className={`flex items-center justify-between rounded-lg px-3 py-2 ${i === 0 ? "bg-[var(--primary)]/10 border border-[var(--primary)]/30" : "bg-[var(--surface-elevated)]"}`}>
                                        <div className="font-arabic-body text-xs text-[var(--text)]">
                                            {b.bidder_name} {b.verified && "✓"} {i === 0 && <span className="text-[var(--primary)] font-bold">(الأعلى)</span>}
                                        </div>
