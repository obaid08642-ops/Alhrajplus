...
function QuickActions() {
    const items = [
        ...
        { to: "/map", icon: "🗺️", label: "خريطة", color: "from-emerald-100 to-green-50 dark:from-emerald-900/20 dark:to-green-900/10" },
    ];
    return (
        ...
        <section className="max-w-7xl mx-auto px-3 sm:px-6">
            <div className="flex items-center justify-between mb-3">
                ...
                <button data-testid="toggle-categories" onClick={onToggle} className="text-xs text-[var(--primary)] font-bold font-arabic flex items-center gap-1">
                    {expanded ? "عرض أقل" : "عرض الكل"} <ChevronDown className={`w-3.5 h-3.5 transition-transform ${expanded ? "rotate-180" : ""}`} />
                </button>
            </div>
        ...
        <section className="max-w-7xl mx-auto px-3 sm:px-6">
            ...
            {loading ? (
                ...
                <div className="space-y-3">
                    {listings.map((l, i) => (
                        <>
                            <WideListingCard key={l.id} listing={l} />
                            {i === 4 && <AdSlotInline key="ad1" />}
                        </>
                    ))}
                </div>
            ) : (
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                    {listings.map((l, i) => (
                        <>
                            ...
                            {(i === 7 || i === 15) && <AdSlotInline key={`ad-${i}`} />}
                        </>
                    ))}
                </div>
            )}
