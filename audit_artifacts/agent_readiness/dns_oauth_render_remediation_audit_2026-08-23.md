...
الـfallback الحالي يحمي TOTP من التخزين النصي لكنه يربط تشفير MFA بمفتاح JWT. أضيف مسار انتقال متوافق: عند ضبط `MFA_ENCRYPTION_KEY` الثابت، يحاول Backend المفتاح الجديد أولًا ثم مفتاح JWT القديم للأسرار السابقة؛ وبعد نجاح رمز TOTP فقط يعيد تشفير السر بالمفتاح الجديد. لذلك يجب نشر commit الانتقال قبل وضع المفتاح، ثم توليد Fernet key واحد ثابت بصيغة URL-safe base64 وحفظه سرًا في Render. لا يجوز توليده تلقائيًا داخل `render.yaml` أو تدويره لاحقًا من دون عملية انتقال مدروسة.
