...
export function AnimalsDetailsBoxMobile({
  form,
  setForm
}) {
  const { t } = useI18n();
  ...
  return <View style={s.box} testID="animals-details-box">
            <Text style={s.title}>🐄 {t("تفاصيل الحيوان")}</Text>

            {/* Row 1 */}
            <View style={s.row}>
                <Cell label={t("نوع الحيوان") + " *"}>
                    <PickerCell value={cf.animal_type} options={ANIMAL_TYPES} onChange={v => set({
          animal_type: v,
          breed: ""
        })} />
                </Cell>
                <Cell label={t("السلالة") + " *"}>
                    <PickerCell value={cf.breed} options={breeds} disabled={!animalType} onChange={v => set({
          breed: v
        })} />
            ...
            <View style={s.row}>
                <Cell label={t("العمر") + " *"}>
                    <PickerCell value={cf.age} options={ANIMAL_OPTS.age} onChange={v => set({
          age: v
        })} />
                </Cell>
                <Cell label={t("الجنس") + " *"}>
                    <PickerCell value={cf.gender} options={ANIMAL_OPTS.gender} onChange={v => set({
          gender: v
        })} />
                </Cell>
            </View>
            ...
            <View style={s.row}>
                <Cell label={t("الحالة الصحية") + " *"}>
                    <PickerCell value={cf.health_status} options={ANIMAL_OPTS.health_status} onChange={v => set({
          health_status: v
        })} />
                </Cell>
                <Cell label={t("التطعيم") + " *"}>
                    <PickerCell value={cf.vaccinated} options={ANIMAL_OPTS.vaccinated} onChange={v => set({
          vaccinated: v
        })} />
                </Cell>
            </View>
          ...
          quantity: v.replace(/[^0-9]/g, "")
            ...
            <View style={s.row}>
                <Cell label={t("الغرض") + " *"}>
                    <PickerCell value={cf.purpose} options={ANIMAL_OPTS.purpose} onChange={v => set({
          purpose: v
        })} />
                </Cell>
          ...
          price: v.replace(/[^0-9.]/g, "")
            ...
            {/* Birds branch */}
            {isBirds && <View style={s.row}>
                    <Cell label={t("نوع القفص / السكن") + " *"}>
                        <PickerCell value={cf.cage_type} options={ANIMAL_OPTS.cage_type} onChange={v => set({
          cage_type: v
        })} />
                    </Cell>
                    <Cell label={t("عدد الطيور بالمجموعة")}>
                        <TextInput value={String(cf.flock_count || "")} onChangeText={v => set({
          flock_count: v.replace(/[^0-9]/g, "")
        })} keyboardType="numeric" placeholderTextColor={colors.textMuted} style={s.input} />
                    </Cell>
                </View>}

            {/* Horse branch */}
            {isHorse && <View style={s.row}>
                    <Cell label={t("مستوى التدريب") + " *"}>
                        <PickerCell value={cf.training_level} options={ANIMAL_OPTS.horse_training_level} onChange={v => set({
          training_level: v
        })} />
                    </Cell>
                    <Cell label={t("الاستخدام") + " *"}>
                        <PickerCell value={cf.horse_usage} options={ANIMAL_OPTS.horse_usage} onChange={v => set({
          horse_usage: v
        })} />
                    </Cell>
                </View>}
...
export function EquipmentDetailsBoxMobile({
  form,
  setForm
}) {
  const { t } = useI18n();
  ...
  return <View style={s.box} testID="equipment-details-box">
            <Text style={s.title}>🏗️ {t("تفاصيل المعدات")}</Text>

            {/* Row 1 */}
            <View style={s.row}>
                <Cell label={t("نوع المعدة") + " *"}>
                    <PickerCell value={cf.equipment_type} options={EQUIPMENT_TYPES} onChange={v => set({
          equipment_type: v
        })} />
                </Cell>
                <Cell label={t("الحالة") + " *"}>
                    <PickerCell value={cf.condition} options={EQUIPMENT_OPTS.condition} onChange={v => set({
          condition: v
        })} />
                </Cell>
            </View>
            ...
            <View style={s.row}>
                <Cell label={t("الماركة") + " *"}>
                    <PickerCell value={cf.brand} options={EQUIPMENT_BRANDS} onChange={v => set({
          brand: v
        })} />
                </Cell>
                <Cell label={t("الموديل") + " *"}>
                    <TextInput value={cf.model || ""} onChangeText={v => set({
          model: v
        })} placeholder={t("مثال: CAT 320D")} placeholderTextColor={colors.textMuted} style={s.input} />
                </Cell>
            </View>
            ...
            <View style={s.row}>
                <Cell label={t("سنة الصنع") + " *"}>
                    <PickerCell value={cf.year} options={EQUIPMENT_OPTS.year} onChange={v => set({
          year: v
        })} />
                </Cell>
                <Cell label={t("ساعات التشغيل")}>
                    <TextInput value={String(cf.usage_hours || "")} onChangeText={v => set({
          usage_hours: v.replace(/[^0-9]/g, "")
        })} keyboardType="numeric" placeholderTextColor={colors.textMuted} style={s.input} />
                </Cell>
            </View>
            ...
            <View style={s.row}>
                <Cell label={t("نوع العرض") + " *"}>
                    <PickerCell value={cf.rental_or_sale} options={EQUIPMENT_OPTS.rental_or_sale} onChange={v => set({
          rental_or_sale: v
        })} />
                </Cell>
          ...
          price: v.replace(/[^0-9.]/g, "")
            ...
            <View style={s.row}>
                <Cell label={t("موقع المعدة") + " *"}>
                    <PickerCell value={cf.location_type} options={EQUIPMENT_OPTS.location_type} onChange={v => set({
          location_type: v
        })} />
                </Cell>
                <Cell label={t("التوافر") + " *"}>
                    <PickerCell value={cf.availability} options={EQUIPMENT_OPTS.availability} onChange={v => set({
          availability: v
        })} />
                </Cell>
            </View>
            ...
            {/* Rental branch */}
            {isRental && <View style={s.row}>
                    <Cell label={t("فترة الإيجار") + " *"}>
                        <PickerCell value={cf.rental_period} options={EQUIPMENT_OPTS.rental_period} onChange={v => set({
          rental_period: v
        })} />
                    </Cell>
                    <Cell label={t("التأمين مطلوب؟") + " *"}>
                        <PickerCell value={cf.insurance_required} options={EQUIPMENT_OPTS.insurance_required} onChange={v => set({
          insurance_required: v
        })} />
                    </Cell>
                </View>}
        </View>;
