...
import { Modal, View, ActivityIndicator, TouchableOpacity, Text, Platform } from "react-native";
...
export default function VoiceCallWebView({ visible, role = "caller", to, convoId, callId, name, signalingEvent, onClose }) {
  ...
  // expo-audio exposes a real earpiece/speaker route switch on Android only.
  const canRouteSpeaker = Platform.OS === "android";
  useEffect(() => {
    if (!visible) { setReady(false); return; }
    setSpeakerEnabled(true);
    setAudioModeAsync({ playsInSilentMode: true, allowsRecording: false, ...(canRouteSpeaker ? { shouldRouteThroughEarpiece: false } : {}) }).catch(() => {});
  ...
  useEffect(() => {
    if (visible && ready && token) {
      ref.current?.postMessage(JSON.stringify({ type: "voice-config", token, backend: BACKEND_URL, iceServers, speakerRoutingSupported: canRouteSpeaker }));
    }
  }, [visible, ready, token, iceServers, canRouteSpeaker]);
  useEffect(() => {
  ...
  const closeCall = () => {
    setAudioModeAsync({ allowsRecording: false, playsInSilentMode: true, ...(canRouteSpeaker ? { shouldRouteThroughEarpiece: false } : {}) }).catch(() => {});
    onClose?.();
  };
  ...
  const url = `https://www.alhraj.online/voice-call.html?v=phase6_call_ui_3&role=${encodeURIComponent(role)}&to=${encodeURIComponent(to || "")}&convo=${encodeURIComponent(convoId || "")}&callId=${encodeURIComponent(callId || `call_${Date.now()}`)}&name=${encodeURIComponent(name || "Haraj Plus")}`;
  return <Modal visible={visible} animationType="slide" onRequestClose={closeCall}>
    <View style={{ flex: 1, backgroundColor: "#0f1a35" }}>
      <WebView ref={ref} originWhitelist={["*"]} source={{ uri: url }} javaScriptEnabled domStorageEnabled allowsInlineMediaPlayback mediaPlaybackRequiresUserAction={false} mediaCapturePermissionGrantType="grantIfSameHostElsePrompt" onLoadEnd={() => setReady(true)} onMessage={event => { try { const msg = JSON.parse(event.nativeEvent.data); if (msg.type === "speaker" && canRouteSpeaker) { const enabled = msg.value !== false; setSpeakerEnabled(enabled); setAudioModeAsync({ playsInSilentMode: true, allowsRecording: false, shouldRouteThroughEarpiece: !enabled }).catch(() => {}); } if (msg.type === "hangup") closeCall(); } catch (_) {} }} style={{ flex: 1, backgroundColor: "#0f1a35" }} />
      <TouchableOpacity onPress={closeCall} style={{ position: "absolute", top: 48, right: 18, width: 42, height: 42, borderRadius: 22, backgroundColor: "rgba(0,0,0,.65)", alignItems: "center", justifyContent: "center" }} accessibilityLabel="إغلاق المكالمة"><X size={20} color="#fff" /></TouchableOpacity>
