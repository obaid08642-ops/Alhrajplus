        ...
        <View style={s.box} testID="jobs-details-box">
            ...
            <View style={s.row}>
                ...
                <Cell label={t("نوع الوظيفة") + " *"}>
                    <PickerCell value={cf.job_type} options={JOB_OPTS.job_type} onChange={(v) => set({ job_type: v })} />
                </Cell>
            </View>
            ...
            <View style={s.row}>
                {isSeeker ? (
                    <Cell label={t("الراتب المتوقع")}>
                        <TextInput value={cf.expected_salary || ""} onChangeText={(v) => set({ expected_salary: v })} placeholder={t("مثال: 8,000 ر.س")} placeholderTextColor={colors.textMuted} style={s.input} />
                    </Cell>
                ) : (
                    <Cell label={t("نطاق الراتب")}>
                        <TextInput value={cf.salary_range || ""} onChangeText={(v) => set({ salary_range: v })} placeholder={t("مثال: 6,000 - 10,000 ر.س")} placeholderTextColor={colors.textMuted} style={s.input} />
                    </Cell>
                )}
            ...
            <View style={s.row}>
                <Cell label={t("المؤهل العلمي") + " *"}>
                    <PickerCell value={cf.education_level} options={JOB_OPTS.education_level} onChange={(v) => set({ education_level: v })} />
                </Cell>
                <Cell label={t("جدول العمل")}>
                    <PickerCell value={cf.work_schedule} options={JOB_OPTS.work_schedule} onChange={(v) => set({ work_schedule: v })} />
                </Cell>
            </View>
            ...
            <View style={s.row}>
                <Cell label={t("نمط الموقع") + " *"}>
                    <PickerCell value={cf.location_type} options={JOB_OPTS.location_type} onChange={(v) => set({ location_type: v })} />
                </Cell>
                <Cell label={t("المجال / التخصص") + " *"}>
                    <PickerCell value={cf.field} options={JOB_OPTS.field} onChange={(v) => set({ field: v })} />
                </Cell>
            </View>
            ...
            {isSeeker ? (
                <View style={{ marginTop: 4 }}>
                    ...
                    <TextInput value={cf.skills || ""} onChangeText={(v) => set({ skills: v })} placeholder={t("اذكر مهاراتك، مثال: Python, React, إدارة فرق...")} placeholderTextColor={colors.textMuted} style={[s.input, { height: 80, textAlignVertical: "top" }]} multiline />
                </View>
            ) : (
                <View style={{ marginTop: 4 }}>
                    <Text style={s.label}>{t("المتطلبات والشروط")}</Text>
                    <TextInput value={cf.requirements || ""} onChangeText={(v) => set({ requirements: v })} placeholder={t("اذكر المؤهلات والمتطلبات الإلزامية...")} placeholderTextColor={colors.textMuted} style={[s.input, { height: 80, textAlignVertical: "top" }]} multiline />
                </View>
            )}
        </View>
    );
        ...
        <View style={s.box} testID="realestate-details-box">
            ...
            <View style={s.row}>
                ...
                <Cell label={t("نوع الإعلان") + " *"}>
                    <PickerCell value={cf.listing_type} options={RE_OPTS.listing_type} onChange={(v) => set({ listing_type: v, deal_type: v })} />
                </Cell>
            </View>
            ...
            <View style={s.row}>
                <Cell label={t("عدد الغرف") + " *"}>
                    <PickerCell value={cf.rooms} options={RE_OPTS.rooms} onChange={(v) => set({ rooms: v })} />
                </Cell>
                <Cell label={t("عدد الحمامات") + " *"}>
                    <PickerCell value={cf.bathrooms} options={RE_OPTS.bathrooms} onChange={(v) => set({ bathrooms: v })} />
                </Cell>
            </View>
            ...
            <View style={s.row}>
                <Cell label={t("المساحة (م²)") + " *"}>
                    <TextInput value={String(cf.area || "")} onChangeText={(v) => set({ area: v.replace(/[^0-9.]/g, "") })} keyboardType="numeric" placeholderTextColor={colors.textMuted} style={s.input} />
                </Cell>
            ...
            <View style={s.row}>
                <Cell label={t("الفرش") + " *"}>
                    <PickerCell value={cf.furnishing} options={RE_OPTS.furnishing} onChange={(v) => set({ furnishing: v })} />
                </Cell>
            ...
            <View style={s.row}>
                <Cell label={t("عمر البناء") + " *"}>
                    <PickerCell value={cf.building_age} options={RE_OPTS.building_age} onChange={(v) => set({ building_age: v })} />
                </Cell>
                <Cell label={t("طريقة الدفع") + " *"}>
                    <PickerCell value={cf.payment_method} options={RE_OPTS.payment_method} onChange={(v) => set({ payment_method: v })} />
                </Cell>
            </View>
        </View>
    );
