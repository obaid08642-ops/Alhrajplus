...
const DUR_TO_DAYS = { "3 أيام": 3, "5 أيام": 5, "7 أيام": 7, "10 أيام": 10, "14 يوم": 14, "30 يوم": 30 };

export function AuctionsDetailsBoxMobile({ form, setForm }) {
    const { t } = useI18n();
    ...
    return (
        <View style={s.box} testID="auctions-details-box">
            <Text style={s.title}>🏷️ {t("تفاصيل المزاد")}</Text>

            <View style={s.row}>
                <Cell label={t("حالة المعروض") + " *"}>
                    <PickerCell value={cf.item_condition} options={AUC_OPTS.item_condition} onChange={(v) => set({ item_condition: v })} />
                </Cell>
                <Cell label={t("نوع التصنيف") + " *"}>
                    <PickerCell value={cf.category_type} options={AUC_OPTS.category_type} onChange={(v) => set({ category_type: v })} />
                </Cell>
            </View>

            <View style={s.row}>
                <Cell label={t("سعر البداية") + ` (${form.currency || "ر.س"}) *`}>
                    <TextInput value={form.price || ""} onChangeText={(v) => setForm({ ...form, price: v.replace(/[^0-9.]/g, ""), custom_fields: { ...form.custom_fields, starting_price: v.replace(/[^0-9.]/g, "") } })} keyboardType="numeric" placeholderTextColor={colors.textMuted} style={s.input} />
                </Cell>
                <Cell label={t("أقل زيادة للمزايدة") + " *"}>
                    <TextInput value={String(cf.bid_increment || "")} onChangeText={(v) => set({ bid_increment: v.replace(/[^0-9.]/g, "") })} keyboardType="numeric" placeholderTextColor={colors.textMuted} style={s.input} />
                </Cell>
            </View>

            <View style={s.row}>
                <Cell label={t("السعر الاحتياطي (سري)")}>
                    <TextInput value={String(cf.reserve_price || "")} onChangeText={(v) => set({ reserve_price: v.replace(/[^0-9.]/g, "") })} keyboardType="numeric" placeholderTextColor={colors.textMuted} style={s.input} />
                </Cell>
                <Cell label={t("اشتر الآن (Buy Now)")}>
                    <TextInput value={String(cf.buy_now_price || "")} onChangeText={(v) => set({ buy_now_price: v.replace(/[^0-9.]/g, "") })} keyboardType="numeric" placeholderTextColor={colors.textMuted} style={s.input} />
                </Cell>
            </View>

            <View style={s.row}>
                <Cell label={t("مدة المزاد") + " *"}>
                    <PickerCell value={cf.auction_duration} options={AUC_OPTS.auction_duration} onChange={(v) => set({ auction_duration: v })} />
                </Cell>
                <Cell label={t("وقت الانتهاء")}>
                    <TextInput value={cf.end_time || ""} onChangeText={(v) => set({ end_time: v })} placeholder="YYYY-MM-DD HH:mm" placeholderTextColor={colors.textMuted} style={s.input} />
                    {cf.auction_duration && cf.auction_duration !== "حسب التاريخ المحدد" && (
                        <Text style={s.hintOk}>✓ {t("تلقائي من المدة")}</Text>
                    )}
                </Cell>
            </View>
            ...
            <View style={s.row}>
                <Cell label={t("خيار الشحن") + " *"}>
                    <PickerCell value={cf.shipping_option} options={AUC_OPTS.shipping_option} onChange={(v) => set({ shipping_option: v })} />
                </Cell>
                <Cell label={t("طريقة الدفع") + " *"}>
                    <PickerCell value={cf.payment_method} options={AUC_OPTS.payment_method} onChange={(v) => set({ payment_method: v })} />
                </Cell>
            </View>
...
export function ServicesProDetailsBoxMobile({ form, setForm }) {
    const { t } = useI18n();
    const cf = form.custom_fields || {};
    const set = (patch) => setForm({ ...form, custom_fields: { ...form.custom_fields, ...patch } });
    ...
    return (
        <View style={s.box} testID="services-pro-details-box">
            <Text style={s.title}>🛠️ {t("تفاصيل الخدمة")}</Text>

            <View style={s.row}>
                <Cell label={t("نوع الخدمة") + " *"}>
                    <PickerCell value={cf.service_type} options={SVC_OPTS.service_type} onChange={(v) => set({ service_type: v })} />
                </Cell>
                <Cell label={t("طريقة التسعير") + " *"}>
                    <PickerCell value={cf.pricing_type} options={SVC_OPTS.pricing_type} onChange={(v) => set({ pricing_type: v })} />
                </Cell>
            </View>
                ...
                <>
                    <View style={s.row}>
                        <Cell label={t("متى تريد الخدمة؟") + " *"}>
                            <PickerCell value={cf.schedule_type} options={SVC_OPTS.schedule_type} onChange={(v) => set({ schedule_type: v })} />
                        </Cell>
                        <View style={s.cell} />
                    </View>
                    {isScheduled && (
                        <View style={s.row}>
                            <Cell label={t("التاريخ")}>
                                <TextInput value={cf.delivery_date || ""} onChangeText={(v) => set({ delivery_date: v })} placeholder="YYYY-MM-DD" placeholderTextColor={colors.textMuted} style={s.input} />
                            </Cell>
                            <Cell label={t("الوقت")}>
                                <PickerCell value={cf.delivery_time_window} options={SVC_OPTS.time_window} onChange={(v) => set({ delivery_time_window: v })} />
                            </Cell>
                        </View>
                    )}
                    <View style={s.row}>
                        <Cell label={t("نقطة الالتقاط") + " *"}>
                            <TextInput value={cf.pickup_location || ""} onChangeText={(v) => set({ pickup_location: v })} placeholder={t("المدينة، الحي")} placeholderTextColor={colors.textMuted} style={s.input} />
                        </Cell>
                        <Cell label={t("نقطة الوصول") + " *"}>
                            <TextInput value={cf.dropoff_location || ""} onChangeText={(v) => set({ dropoff_location: v })} placeholder={t("المدينة، الحي")} placeholderTextColor={colors.textMuted} style={s.input} />
                        </Cell>
                    </View>
                    <View style={s.row}>
                        <Cell label={t("نوع المركبة") + " *"}>
                            <PickerCell value={cf.vehicle_type} options={SVC_OPTS.vehicle_type} onChange={(v) => set({ vehicle_type: v })} />
                        </Cell>
                        <Cell label={t("حجم / وزن الحمولة")}>
                            <PickerCell value={cf.load_type} options={SVC_OPTS.load_type} onChange={(v) => set({ load_type: v })} />
                        </Cell>
                    </View>
                    <View style={s.row}>
                        <Cell label={t("السعر المقترح")}>
                            <TextInput value={String(cf.price_estimate || "")} onChangeText={(v) => set({ price_estimate: v.replace(/[^0-9.]/g, "") })} keyboardType="numeric" placeholderTextColor={colors.textMuted} style={s.input} />
                        </Cell>
                        <Cell label={t("المسافة (كم)")}>
                            <TextInput value={String(cf.distance_km || "")} onChangeText={(v) => set({ distance_km: v.replace(/[^0-9.]/g, "") })} keyboardType="numeric" placeholderTextColor={colors.textMuted} style={s.input} />
                        </Cell>
                    </View>
                    <View style={s.row}>
                        <Cell label={t("التكرار")}>
                            <PickerCell value={cf.delivery_recurrence} options={SVC_OPTS.delivery_recurrence} onChange={(v) => set({ delivery_recurrence: v })} />
                        </Cell>
                        <Cell label={t("نوع الشحنة")}>
                            <PickerCell value={cf.package_type} options={SVC_OPTS.package_type} onChange={(v) => set({ package_type: v })} />
                        </Cell>
                    </View>
                </>
            )}
                ...
                <>
                    <View style={s.row}>
                        <Cell label={t("نوع العقار") + " *"}>
                            <PickerCell value={cf.property_type} options={SVC_OPTS.property_type} onChange={(v) => set({ property_type: v })} />
                        </Cell>
                        <Cell label={t("عدد الغرف")}>
                            <PickerCell value={cf.rooms} options={SVC_OPTS.rooms} onChange={(v) => set({ rooms: v })} />
                        </Cell>
                    </View>
                    <View style={s.row}>
                        <Cell label={t("مدة الخدمة") + " *"}>
                            <PickerCell value={cf.service_duration} options={SVC_OPTS.service_duration} onChange={(v) => set({ service_duration: v })} />
                        </Cell>
                        <Cell label={t("التكرار") + " *"}>
                            <PickerCell value={cf.frequency} options={SVC_OPTS.frequency} onChange={(v) => set({ frequency: v })} />
                        </Cell>
                    </View>
                    <View style={s.row}>
                        <Cell label={t("الوقت المفضل")}>
                            <PickerCell value={cf.preferred_time} options={SVC_OPTS.preferred_time} onChange={(v) => set({ preferred_time: v })} />
                        </Cell>
                        <Cell label={t("المواد متضمنة؟")}>
                            <PickerCell value={cf.materials_included} options={SVC_OPTS.materials_included} onChange={(v) => set({ materials_included: v })} />
                        </Cell>
                    </View>
                </>
            )}
                ...
                <>
                    <View style={s.row}>
                        <Cell label={t("نوع المشروع") + " *"}>
                            <PickerCell value={cf.project_type} options={SVC_OPTS.project_type} onChange={(v) => set({ project_type: v })} />
                        </Cell>
                        <Cell label={t("مستوى الخبرة") + " *"}>
                            <PickerCell value={cf.experience_level} options={SVC_OPTS.experience_level} onChange={(v) => set({ experience_level: v })} />
                        </Cell>
                    </View>
                    <View style={s.row}>
                        <Cell label={t("مدة التسليم") + " *"}>
                            <PickerCell value={cf.delivery_time} options={SVC_OPTS.delivery_time} onChange={(v) => set({ delivery_time: v })} />
                        </Cell>
                        <Cell label={t("التعديلات")}>
                            <PickerCell value={cf.revisions} options={SVC_OPTS.revisions} onChange={(v) => set({ revisions: v })} />
                        </Cell>
                    </View>
                    <View style={s.row}>
                        <Cell label={t("النطاق المالي")}>
                            <PickerCell value={cf.budget_range} options={SVC_OPTS.budget_range} onChange={(v) => set({ budget_range: v })} />
                        </Cell>
                        <Cell label={t("وسيلة التواصل")}>
                            <PickerCell value={cf.communication_method} options={SVC_OPTS.communication_method} onChange={(v) => set({ communication_method: v })} />
                        </Cell>
                    </View>
                </>
            )}
            ...
            {isEdu && (
                <>
                    <View style={s.row}>
                        <Cell label={t("المادة") + " *"}>
                            <PickerCell value={cf.edu_subject} options={SVC_OPTS.edu_subject} onChange={(v) => set({ edu_subject: v })} />
                        </Cell>
                        <Cell label={t("المستوى") + " *"}>
                            <PickerCell value={cf.edu_level} options={SVC_OPTS.edu_level} onChange={(v) => set({ edu_level: v })} />
                        </Cell>
                    </View>
                    <View style={s.row}>
                        <Cell label={t("مدة الجلسة") + " *"}>
                            <PickerCell value={cf.session_duration} options={SVC_OPTS.session_duration} onChange={(v) => set({ session_duration: v })} />
                        </Cell>
                        <Cell label={t("عدد الجلسات")}>
                            <PickerCell value={cf.sessions_count} options={SVC_OPTS.sessions_count} onChange={(v) => set({ sessions_count: v })} />
                        </Cell>
                    </View>
                    <View style={s.row}>
                        <Cell label={t("نمط التدريس") + " *"}>
                            <PickerCell value={cf.delivery_mode} options={SVC_OPTS.delivery_mode} onChange={(v) => set({ delivery_mode: v })} />
                        </Cell>
                        <Cell label={t("الجدول الزمني")}>
                            <PickerCell value={cf.edu_schedule} options={SVC_OPTS.edu_schedule} onChange={(v) => set({ edu_schedule: v })} />
                        </Cell>
                    </View>
                </>
            )}
            ...
            {!sType && (
                <Text style={s.emptyHint}>💡 {t("اختر «نوع الخدمة» أعلاه لتظهر الحقول التفصيلية الخاصة بها.")}</Text>
            )}
        </View>
    );
}
...
const s = StyleSheet.create({
    ...
    hintOk: { fontSize: 9, color: "#10b981", marginTop: 3 },
    warn: { fontSize: 10, color: "#f59e0b", fontWeight: "700", marginTop: 2 },
    emptyHint: { fontSize: 10, color: colors.textMuted, marginTop: 4, fontStyle: "italic" },
