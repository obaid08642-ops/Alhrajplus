...
export default function ListingCard({ listing, wide = false }) {
    ...
    const toggleFav = async (e) => {
        ...
        try {
            if (fav) { await api.delete(`/favorites/${listing.id}`); setFav(false); }
            else { await api.post(`/favorites/${listing.id}`); setFav(true); }
        } catch (_) {}
    };
    ...
    if (wide) {
        return (
            <TouchableOpacity activeOpacity={0.9} onPress={() => nav.navigate("ListingDetail", { id: listing.id })} style={[styles.wide, shadow.card]}>
                <View style={styles.wideImgBox}>
                    {img ? <Image source={{ uri: img }} style={styles.wideImg} /> : <View style={styles.imgPlaceholder} />}
                </View>
        ...
        <TouchableOpacity activeOpacity={0.85} onPress={() => nav.navigate("ListingDetail", { id: listing.id })} style={[styles.card, shadow.card]}>
            ...
            <View style={styles.body}>
                ...
                <View style={styles.cityRow}>
                    ...
                    <Text style={styles.muted} numberOfLines={1}>{listing.city || ""}</Text>
                    {listing.verified && <BadgeCheck size={12} color={colors.primary} style={{ marginStart: 4 }} />}
                </View>
            </View>
        </TouchableOpacity>
    );
...
const styles = StyleSheet.create({
    ...
    imgBox: { aspectRatio: 4 / 3, backgroundColor: colors.surfaceElevated, position: "relative" },
    img: { width: "100%", height: "100%" },
    imgPlaceholder: { width: "100%", height: "100%", backgroundColor: colors.surfaceElevated },
    favBtn: {
    ...
    footer: { flexDirection: "row", alignItems: "baseline", justifyContent: "space-between", marginTop: 2 },
    price: { fontSize: 15, fontWeight: "900", color: colors.primary },
    currency: { fontSize: 10, fontWeight: "600", color: colors.primary },
    muted: { fontSize: 10, color: colors.textMuted, fontWeight: "500" },
    ...
    wideImgBox: { width: 120, height: 120, backgroundColor: colors.surfaceElevated },
    wideImg: { width: "100%", height: "100%" },
    wideBody: { flex: 1, padding: 12, justifyContent: "space-between" },
    wideFoot: { flexDirection: "row", alignItems: "baseline", justifyContent: "space-between", marginTop: 6 },
});
