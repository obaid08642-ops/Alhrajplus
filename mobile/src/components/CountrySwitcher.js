    ...
    return (
        <>
            <TouchableOpacity
                ...
                testID="country-switcher-btn"
                activeOpacity={0.85}
                hitSlop={6}
            >
                {current ? (
                    <Text style={[styles.flag, tintLight && { color: "#fff" }]}>{current.flag}</Text>
                ) : (
                    ...
                    <View style={styles.sheet}>
                        ...
                        <FlatList
                            ...
                            renderItem={({ item }) => {
                                ...
                                return (
                                    <TouchableOpacity
                                        onPress={async () => { await setCountry(item.code); setOpen(false); }}
                                        style={[styles.row, isCur && styles.rowActive]}
                                        testID={`country-pick-${item.code}`}
                                    >
                                        <Text style={styles.rowFlag}>{item.flag}</Text>
                                        <View style={{ flex: 1 }}>
                                            <Text style={[styles.rowName, isCur && { color: colors.primary, fontWeight: "900" }]}>{item.name_ar}</Text>
...
const styles = StyleSheet.create({
    ...
    flag: { fontSize: 16 },
    code: { fontSize: 11, fontWeight: "800", color: colors.text, maxWidth: 80 },

    sheetBg: { flex: 1, backgroundColor: "rgba(0,0,0,0.55)", justifyContent: "flex-end" },
    sheet: { backgroundColor: colors.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 14, maxHeight: "75%" },
    sheetHead: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 4, paddingBottom: 10 },
    ...
    rowActive: { backgroundColor: "rgba(79,182,230,0.08)" },
    rowFlag: { fontSize: 26 },
