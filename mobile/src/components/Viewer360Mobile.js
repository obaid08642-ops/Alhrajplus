...
export default function Viewer360Mobile({
  images = [],
  onClose
}) {
  const {
  ...
  return <View style={styles.wrap} {...panResponder.panHandlers} testID="viewer-360-mobile">
            {images.map((src, i) => <Image key={src + i} source={{
      uri: src
    }} resizeMode="contain" style={[styles.frame, {
      opacity: i === index ? 1 : 0
    }]} />)}
