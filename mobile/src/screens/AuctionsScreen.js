  ...
  return <ScrollView style={{
    ...
    load();
      ...
      marginTop: 30
            ...
            {active && <BidModal user={user} listing={active} onClose={() => setActive(null)} onPlaced={() => {
      setActive(null);
      load();
    }} />}
        </ScrollView>;
...
function AuctionCard({
  ...
  return <View style={[styles.card, shadow.card]}>
            <TouchableOpacity onPress={() => nav.navigate("ListingDetail", {
      id: listing.id
    })} activeOpacity={0.9}>
        ...
        paddingBottom: Math.max(insets.bottom, 16)
          ...
          gap: 12
            ...
            opacity: 0.5
                        ...
                        {bids.length > 0 && <View style={{
            marginTop: 6
          }}>
                                <Text style={styles.label}>{t("تاريخ المزايدات")}</Text>
                                <FlatList data={bids} keyExtractor={b => b.id} style={{
              maxHeight: 180,
              marginTop: 6
            }} renderItem={({
              item,
              index
                                            ...
                                            <Text style={styles.bidName}>{item.bidder_name} {item.verified && "✓"} {index === 0 && <Text style={{
