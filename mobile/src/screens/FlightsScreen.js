  ...
  return <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
            <View style={pStyles.bg}>
                <View style={pStyles.sheet}>
                    ...
                    <FlatList data={filtered} keyExtractor={a => a.code} renderItem={({
          item
        }) => <TouchableOpacity onPress={() => {
          onPick(item.code);
          onClose();
          setQ("");
        }} style={[pStyles.row, item.code === current && pStyles.rowActive]}>
                                <View style={{
            flex: 1
          }}>
...
export default function FlightsScreen() {
  ...
  const search = provider => {
    if (!from || !to || !date) {
      Alert.alert(t("تنبيه"), t("الرجاء اختيار المطار والتاريخ"));
      return;
    }
    if (from === to) {
      Alert.alert(t("تنبيه"), t("لا يمكن أن يكون المغادرة والوصول متشابهين"));
      return;
    }
    const dDay = fmtYYMMDD(date);
    const rDay = fmtYYMMDD(returnDate);
    let url = "";
    if (provider === "skyscanner") url = `https://www.skyscanner.net/transport/flights/${from.toLowerCase()}/${to.toLowerCase()}/${dDay}${tripType === "round" && rDay ? `/${rDay}` : ""}/?adults=${pax}&currency=SAR&utm_source=harajplus`;else if (provider === "wego") url = `https://www.wego.com/flights/searches/${from}-${to}${tripType === "round" && returnDate ? `-${returnDate}` : ""}-${date}/economy/${pax}adults`;else if (provider === "kayak") url = `https://www.kayak.com/flights/${from}-${to}/${date}${tripType === "round" && returnDate ? `/${returnDate}` : ""}/${pax}adults?currency=SAR`;else if (provider === "googleFlights") url = `https://www.google.com/travel/flights?q=Flights%20to%20${to}%20from%20${from}%20on%20${date}${tripType === "round" && returnDate ? `%20returning%20${returnDate}` : ""}`;else if (provider === "trip") url = `https://www.trip.com/flights/showfarefirst?dcity=${from}&acity=${to}&ddate=${date}${tripType === "round" && returnDate ? `&rdate=${returnDate}` : ""}&triptype=${tripType === "round" ? "rt" : "ow"}&class=y&quantity=${pax}&Allianceid=8199633&SID=309959147&trip_sub1=alhraj`;
  ...
  const onDateChange = (event, selected) => {
    ...
    if (which === "go") setDate(iso);else if (which === "back") setReturnDate(iso);
  };
    ...
    paddingBottom: 130
        ...
        marginTop: 10
                ...
                <View style={styles.providersGrid}>
                    {PROVIDERS.map(p => <TouchableOpacity key={p.key} onPress={() => search(p.key)} style={[styles.providerBtn, p.full && {
          width: "100%"
        }]} activeOpacity={0.9}>
                            <LinearGradient colors={p.bg} style={StyleSheet.absoluteFillObject} start={{
            x: 0,
            y: 0
          }} end={{
            x: 1,
      ...
      if (pickerFor === "from") setFrom(c);else setTo(c);
    }} onClose={() => setPickerFor(null)} />
            {showDate && <DateTimePicker value={showDate === "go" ? date ? new Date(date) : new Date() : returnDate ? new Date(returnDate) : date ? new Date(date) : new Date()} mode="date" display={Platform.OS === "ios" ? "spinner" : "default"} minimumDate={showDate === "back" && date ? new Date(date) : new Date()} onChange={onDateChange} />}
        </ScrollView>;
}
