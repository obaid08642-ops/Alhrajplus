...
export default function WalletScreen() {
  ...
  const claimBonus = async () => {
    ...
    try {
      const {
        data: r
      } = await api.post("/wallet/claim-welcome-bonus");
  ...
  if (!user) {
    return <View style={styles.centerWrap}>
    ...
    load();
        ...
        padding: 22
          ...
          marginBottom: 6
            ...
            {/* Welcome bonus */}
            {!hasBonus && <View style={styles.bonusCard}>
      ...
      marginTop: 12
                ...
                <TouchableOpacity onPress={() => nav.navigate("MyListings")} style={[styles.quickCard, {
        flex: 1
      }]}>
      ...
      marginTop: 14
        ...
        padding: 24
      }} /> : (data.transactions || []).length === 0 ? <Text style={styles.emptyText}>{t("لا توجد عمليات بعد")}</Text> : data.transactions.map((tx, i) => {
        const positive = (tx.amount || 0) > 0;
        return <View key={tx.id || i} style={[styles.txRow, i === data.transactions.length - 1 && {
          borderBottomWidth: 0
        }]}>
