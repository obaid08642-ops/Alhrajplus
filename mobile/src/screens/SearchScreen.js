...
export default function SearchScreen({ navigation, route }) {
    ...
    useEffect(() => {
        api.get("/meta/categories", { params: { lang } }).then(({ data }) => setCategories(data || [])).catch(() => {});
    }, [lang]);
        ...
        <View style={{ flex: 1, backgroundColor: colors.bg }}>
            ...
            <View style={[s.header, { paddingTop: insets.top + 6 }]}>
                ...
                <View style={s.searchPill}>
                    ...
                    <TextInput
                        value={q}
                        onChangeText={(v) => { setQ(v); setShowSugg(true); }}
            ...
            {showSugg && suggestions.length > 0 && (
                <View style={s.suggBox}>
                    {suggestions.map((sug, i) => {
                        ...
                        return (
                            <TouchableOpacity key={i} onPress={() => { setQ(text); runSearch(text); }} style={s.suggRow}>
            ...
            {loading ? (
                ...
                <View style={s.empty}>
                    <Search size={48} color={colors.textMuted} style={{ opacity: 0.5 }} />
                ...
                <FlatList
                    ...
                    refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); runSearch(); }} tintColor={colors.primary} />}
                    renderItem={({ item }) => (
...
function FiltersModal({ visible, onClose, filters, setFilters, categories, country, onApply, onClear }) {
    const { t } = useI18n();
    const [local, setLocal] = useState(filters);
    useEffect(() => { if (visible) setLocal(filters); }, [visible, filters]);

    const apply = () => { setFilters(local); onApply(); };
        ...
        <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
            <View style={s.modalBg}>
                <View style={s.modalSheet}>
                    ...
                    <ScrollView style={{ maxHeight: "75%" }} contentContainerStyle={{ padding: 14 }}>
                        {/* Sort */}
                        <Text style={s.sectionLabel}>{t("الترتيب")}</Text>
                        <View style={s.tagsRow}>
                            {SORT_OPTIONS.map((opt) => (
                                <TouchableOpacity key={opt.key} onPress={() => setLocal({ ...local, sort: opt.key })} style={[s.tag, local.sort === opt.key && s.tagActive]}>
                        ...
                        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                            <View style={{ flexDirection: "row", gap: 6, paddingVertical: 2 }}>
                                {(country?.cities || []).slice(0, 20).map((c) => (
                                    <TouchableOpacity key={c.name_ar} onPress={() => setLocal({ ...local, city: local.city === c.name_ar ? "" : c.name_ar })} style={[s.tag, local.city === c.name_ar && s.tagActive]}>
                        ...
                        <View style={s.tagsRow}>
                            {categories.map((c) => (
                                <TouchableOpacity key={c.key} onPress={() => setLocal({ ...local, category: local.category === c.key ? "" : c.key })} style={[s.tag, local.category === c.key && s.tagActive]}>
                    ...
                    <View style={s.modalFoot}>
                        <TouchableOpacity onPress={() => { onClear(); setLocal({ category: "", city: "", priceMin: "", priceMax: "", condition: "", sort: "newest" }); }} style={s.clearBtn}>
...
const s = StyleSheet.create({
    header: { flexDirection: "row", alignItems: "center", gap: 8, paddingHorizontal: 8, paddingBottom: 10, backgroundColor: colors.bg },
    headBtn: { padding: 6 },
    searchPill: { flex: 1, flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: colors.surface, borderRadius: 999, borderWidth: 1, borderColor: colors.border, paddingHorizontal: 14, paddingVertical: 8 },
    searchInput: { flex: 1, color: colors.text, fontSize: 14, paddingVertical: 2 },
    filterBtn: { width: 42, height: 42, borderRadius: 999, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, alignItems: "center", justifyContent: "center", position: "relative" },
    filterBtnActive: { backgroundColor: colors.primary, borderColor: colors.primary },
    filterBadge: { position: "absolute", top: -2, end: -2, minWidth: 18, height: 18, borderRadius: 999, backgroundColor: colors.accent, alignItems: "center", justifyContent: "center", paddingHorizontal: 4 },
    filterBadgeText: { fontSize: 10, fontWeight: "900", color: colors.secondary },
    chipRow: { flexDirection: "row", gap: 6, paddingHorizontal: 12, paddingBottom: 8 },
    chip: { flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "rgba(79,182,230,0.12)", borderColor: colors.primary, borderWidth: 1, borderRadius: 999, paddingHorizontal: 10, paddingVertical: 5 },
    ...
    modalHead: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", padding: 16, borderBottomWidth: 1, borderColor: colors.border },
    modalTitle: { fontSize: 17, fontWeight: "900", color: colors.text },
    modalCloseX: { width: 32, height: 32, borderRadius: 999, backgroundColor: colors.surfaceElevated, alignItems: "center", justifyContent: "center" },
    sectionLabel: { fontSize: 13, fontWeight: "800", color: colors.text, marginTop: 14, marginBottom: 8 },
    tagsRow: { flexDirection: "row", flexWrap: "wrap", gap: 6 },
    tag: { backgroundColor: colors.surfaceElevated, paddingHorizontal: 12, paddingVertical: 7, borderRadius: 999, borderWidth: 1, borderColor: colors.border },
    tagActive: { backgroundColor: colors.primary, borderColor: colors.primary },
    tagText: { fontSize: 12, fontWeight: "700", color: colors.text },
    tagTextActive: { color: "#fff" },
    priceInput: { flex: 1, backgroundColor: colors.surfaceElevated, borderRadius: 12, borderWidth: 1, borderColor: colors.border, paddingHorizontal: 14, paddingVertical: 10, fontSize: 14, color: colors.text },
    priceDash: { alignSelf: "center", color: colors.textMuted, fontSize: 16 },
    modalFoot: { flexDirection: "row", gap: 10, padding: 14, borderTopWidth: 1, borderColor: colors.border },
    clearBtn: { flex: 1, backgroundColor: colors.surfaceElevated, borderRadius: 14, paddingVertical: 12, alignItems: "center" },
    clearText: { color: colors.textMuted, fontWeight: "800", fontSize: 13 },
    applyBtn: { flex: 2, backgroundColor: colors.primary, borderRadius: 14, paddingVertical: 12, alignItems: "center", flexDirection: "row", justifyContent: "center", gap: 6 },
    applyText: { color: "#fff", fontWeight: "900", fontSize: 14 },
});
