...
export default function AIAssistantScreen() {
  ...
  const send = async textOverride => {
    ...
    try {
      ...
      setMessages([...next, {
        role: "assistant",
        text: `⚠️ ${typeof err === "string" ? err : t("خطأ")}`
      }]);
    } finally {
      setBusy(false);
    }
  ...
  return <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{
    ...
    backgroundColor: colors.bg
            ...
            <View style={styles.header}>
                <LinearGradient colors={[colors.primary, colors.accent]} style={StyleSheet.absoluteFillObject} start={{
        x: 0,
        y: 0
      }} end={{
        x: 1,
        y: 1
          ...
          flex: 1
                    ...
                    {messages.length > 0 && <TouchableOpacity onPress={reset} style={styles.resetBtn}>
            ...
            {/* Messages */}
            {messages.length === 0 ? <View style={styles.empty}>
                    <Sparkles size={40} color={colors.primary} style={{
        opacity: 0.5
      }} />
                    <Text style={styles.emptyTitle}>{t("اقتراحات سريعة:")}</Text>
                    {SUGGESTIONS.map((s, i) => <TouchableOpacity key={i} onPress={() => send(s)} style={styles.suggBtn}>
                            <Text style={styles.suggText}>{s}</Text>
                        </TouchableOpacity>)}
                </View> : <FlatList ref={listRef} data={messages} keyExtractor={(_, i) => String(i)} contentContainerStyle={{
      padding: 12
    }} renderItem={({
      item
    }) => {
      const isUser = item.role === "user";
      return <View style={[styles.bubbleWrap, {
        alignItems: isUser ? "flex-end" : "flex-start"
      }]}>
                                <View style={[styles.bubble, isUser ? styles.userBubble : styles.botBubble]}>
                                    <Text style={[styles.bubbleText, isUser && {
            color: "#fff"
          }]}>{item.text}</Text>
                                </View>
                            </View>;
    }} ListFooterComponent={busy ? <View style={[styles.bubbleWrap, {
        ...
        alignItems: "center"
          ...
          fontSize: 12
            ...
            <View style={styles.inputBar}>
                <TextInput value={input} onChangeText={setInput} placeholder={t("اكتب رسالتك...")} placeholderTextColor={colors.textMuted} style={styles.input} editable={!busy} multiline maxLength={2000} />
                <TouchableOpacity onPress={() => send()} disabled={busy || !input.trim()} style={[styles.sendBtn, (busy || !input.trim()) && {
        opacity: 0.5
      }]}>
