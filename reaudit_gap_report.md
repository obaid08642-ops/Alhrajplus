...
### تدقيق الترجمة الشامل على staging

أظهر التدقيق الحي أن الصفحة المنشورة كانت تعرض نصوصًا إنجليزية مع بقاء عبارات عربية، ثم أظهر التدقيق الثابت وجود 505 مفتاحًا عربيًا مستخدمًا في Web وغير موجود في dictionary. تمت ترجمة وإضافة جميع المفاتيح إلى الإنجليزية والأوردية والهندية والبنغالية والفرنسية، بما فيها التواصل مع البائع، إظهار المساعد الذكي، رفع/نشر الستوري، حالات الإدارة، حقول الفئات، رسائل التعليقات، ورسائل الأخطاء. أعاد التدقيق النتيجة `MISSING_COUNT=0`، وصحح Web production build بنجاح. هذه الدفعة تحتاج deployment جديد على الخادم قبل إعادة اختبارها على الرابط الحي.
